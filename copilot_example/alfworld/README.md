...
```bash
code copilot_example/alfworld
# in Copilot Chat (Agent mode):
# /skillopt-loop rounds=3 batch=8
```

One-shot eval only (no chat):

```bash
bash copilot_example/alfworld/run.sh --eval_limit 5 --limit 5
# → copilot_example/alfworld/workspaces/<model>/{skill.md, .skillopt/samples/}
```
