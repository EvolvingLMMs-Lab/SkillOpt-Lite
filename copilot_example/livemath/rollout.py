...
from skillopt.model import chat_target, chat_target_messages, get_target_backend, is_target_exec_backend
from skillopt.model.codex_harness import prepare_workspace, render_skill_md, run_target_exec
from skillopt.prompts import load_prompt

# Optional harness extensions removed in the public release.
# Provide inline no-op stubs so the rollout still works. The router and
# sympy tool loop were OFF by default anyway (gated on SKILLOPT_LIVEMATH_*
# env vars).
def tools_enabled() -> bool:  # noqa: D401
    return False


def router_enabled() -> bool:  # noqa: D401
    return False


def maybe_route_skill(item, skill_content):  # noqa: D401
    return skill_content, "default"


def _chat_step(*, messages, max_completion_tokens, timeout, retries, stage):
    # Fallback: plain chat completion, matching chat_target_messages semantics.
    resp = chat_target_messages(
        messages=messages,
...
def process_one(
    ...
    *,
    ...
    try:
        ...
        # Optional skill router (no-op when SKILLOPT_LIVEMATH_ROUTER unset).
        skill_content, route_label = maybe_route_skill(item, skill_content)
        if route_label != "default":
        ...
        for turn in range(max_turns):
            ...
            # `_chat_step` mutates `messages` (appends assistant/tool turns).
            # When tools are disabled, it's a thin wrapper over chat_target_messages
            # and we must append the assistant message ourselves for parity.
            tools_active = tools_enabled()
            resp_text, tool_trace = _chat_step(
                messages=messages,
                max_completion_tokens=max_completion_tokens,
                timeout=exec_timeout,
                retries=5,
            ...
            if tool_trace:
                tool_trace_all.extend(tool_trace)
        ...
        if tool_trace_all:
            with open(os.path.join(pred_dir, "tool_trace.json"), "w") as f:
                json.dump(tool_trace_all, f, ensure_ascii=False, indent=2)
            result["n_tool_calls"] = len(tool_trace_all)
