...
_TOOL_DESCRIPTIONS = {
    "glob": "Find candidate local document files by filename or relative-path glob pattern.",
    "read": "Read a local text document excerpt by path and line window.",
    "grep": "Search a local text document for a literal pattern and return matching lines.",
}


def _make_tool_schemas() -> list[dict]:
    """Build the tool schema list exposing glob/read/grep."""
...
_TOOL_SCHEMAS = _make_tool_schemas()
...
def _build_system(
    ...
    *,
    ...
    return load_prompt(
        "rollout_system", env="officeqa"
    ).format(skill_section=skill_section)
