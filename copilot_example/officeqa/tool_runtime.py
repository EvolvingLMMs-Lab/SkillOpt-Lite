...
def resolve_docs_roots(data_dirs: list[str] | tuple[str, ...] | str | None = None) -> list[str]:
    # NOTE: __file__ is at <repo>/copilot_example/officeqa/tool_runtime.py
    # (3 levels deep), unlike the original at <repo>/skillopt/envs/officeqa/...
    # which used parents[3]. Don't blindly copy that index back.
...
_CALC_ALLOWED_NAMES = {
    ...
    "exp": math.exp, "fabs": math.fabs, "floor": math.floor, "ceil": math.ceil,
    "pow": pow, "pi": math.pi, "e": math.e,
    "sum": sum, "min": min, "max": max, "abs": abs, "round": round, "len": len,
    "mean": statistics.mean, "stdev": statistics.stdev, "pstdev": statistics.pstdev,
    "median": statistics.median, "variance": statistics.variance,
}
_CALC_ALLOWED_NODES = (
    ast.Expression, ast.Constant, ast.Num, ast.Name, ast.Load,
    ast.BinOp, ast.UnaryOp, ast.Tuple, ast.List, ast.Call,
...
def _safe_calc(expr: str) -> str:
    try:
        ...
        return f"[calc error: syntax: {e.msg}]"
    for node in ast.walk(tree):
        if not isinstance(node, _CALC_ALLOWED_NODES):
            return f"[calc error: disallowed syntax: {type(node).__name__}]"
        if isinstance(node, ast.Name) and node.id not in _CALC_ALLOWED_NAMES:
            return f"[calc error: unknown name: {node.id}]"
        if isinstance(node, ast.Call) and not (isinstance(node.func, ast.Name) and node.func.id in _CALC_ALLOWED_NAMES):
            return "[calc error: only whitelisted function calls allowed]"
    try:
        result = eval(compile(tree, "<calc>", "eval"), {"__builtins__": {}}, _CALC_ALLOWED_NAMES)
...
def run_tool(name: str, arguments: dict, *, allowed_roots: list[str], allowed_files: list[str]) -> tuple[str, str]:
    if name == "calc":
        ...
        if not expr:
            return "calc(expr='')", "[calc error: missing expr]"
        return f"calc(expr={expr!r})", _safe_calc(expr)
