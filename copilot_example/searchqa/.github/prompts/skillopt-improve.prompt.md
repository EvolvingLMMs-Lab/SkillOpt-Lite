---
description: "Read searchqa rollout samples and improve workspace/skill.md (one round, no re-eval)"
agent: "agent"
tools: ['codebase', 'editFiles', 'runCommands']
---

You are running **one round** of SkillOpt improve on the searchqa skill.

## Inputs (paths are relative to this workspace, `copilot_example/searchqa/`)
...
2. **Explore the samples** (use your file tools — `list_dir`, `read_file`):
   ...
   - Decide your reading strategy based on what you see:
     - **Cluster first, sample second.** Skim filenames / first lines
       (cheap) to group failed samples by symptom (e.g. similar question
       stems, same answer-tag pattern). Then sample 1-2 representatives
...
3. **Diagnose** the most consistent weakness.
   ...
   **Patch discipline** (mirrors `skillopt/prompts/analyst_*.md`):
   ...
   - **Generalizable**: never hardcode task-specific values (entity names,
     exact strings from one question). Edits must read like rules.
...
5. **Sanity-check**: re-read `workspace/skill.md` after the edit and confirm
   ...
   (esp. that you haven't broken the `<answer>` tag instruction).
...
### Expected impact
- Failure modes this addresses (which clusters from copilot-instructions).
- Regressions to watch for (especially answer-format / span-length).
...
- Target model is **`gpt-5.4`** (configured in `run.sh`). Don't change it.
