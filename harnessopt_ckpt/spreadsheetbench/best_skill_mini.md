...
**Same rule when the instruction says *"modify my formula" / "fix this formula" /
"what should my formula be"*.** The user is describing intent, not asking you to
store text. Compute the resulting value in Python and write the literal value.
Writing `"INDEX(...)"` or `"=INDEX(...)"` (with or without the leading `=`) as
the cell value causes the grader to read back the raw string, not the number
you intended → task fails.

When the instruction asks for a **text/string date-time format** (e.g.
`dd/mm/yyyy`, `hh:mm`, `mm-yy`), write the FORMATTED STRING via
`dt.strftime("%d/%m/%Y")` — do NOT write a `datetime`/`date`/`time` object and
call it done by setting `cell.number_format = "dd/mm/yyyy"`. `number_format`
only changes display in Excel; `cell.value` is still the datetime, and the
grader compares `cell.value` — a `datetime` will not equal a `'21/11/21 14:28'`
string.
...
After saving, **reload the output and PRINT the target cells** before you
...
**Never add `assert cell.value == <a value you guessed>` at the end of the
script.** You do not know the gold answers; hard-coding your own guess into
an `assert` will crash the script whenever your guess is wrong, destroying
otherwise-usable output. Only use `print(...)` for verification.
...
- Use exact cell coordinates (e.g. `ws["B2"]`); never pass invalid refs like
  ...
  In particular, **sheet names are NOT cell coordinates** — never write
  `ws[sheet_name]` or `ws["b2b"]` (where `b2b` is a sheet name). To address
  a cell in another sheet, always go through `wb[sheet_name]["A1"]` or
  `wb[sheet_name].cell(row=r, column=c)`.
