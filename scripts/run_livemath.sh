#!/usr/bin/env bash
# ──────────────────────────────────────────────────────────────────────────────
# SkillOpt — LiveMathematicianBench training launch script (TRAPI edition)
#
# One-key per-model launchers live alongside this file:
#   scripts/run_livemath_gpt-5.4.sh
#   scripts/run_livemath_gpt-5.4-mini.sh
#   scripts/run_livemath_gpt-5.4-nano.sh
#   scripts/run_livemath_gpt-5.5.sh
#   scripts/run_livemath_gpt-4o.sh
#
# Usage:
#   bash scripts/run_livemath.sh                              # defaults (gpt-5.5)
#   bash scripts/run_livemath.sh --target_model gpt-5.4-mini
#   TARGET_MODEL=gpt-5.4 bash scripts/run_livemath.sh
# ──────────────────────────────────────────────────────────────────────────────
set -euo pipefail
...
OUT_ROOT="${PROJECT_ROOT}/outputs/skillopt_livemath_${TARGET_MODEL_ALIAS}_${TIMESTAMP}"

echo "============================================================"
echo "  SkillOpt — LiveMathematicianBench Training (TRAPI)"
echo "============================================================"
