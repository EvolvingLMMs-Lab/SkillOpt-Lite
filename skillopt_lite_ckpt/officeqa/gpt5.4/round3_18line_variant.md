...
- Your final assistant message MUST contain `<answer>VALUE</answer>` exactly once. **Never** emit an empty answer. **Never** put narrative sentences inside the tags — only the bare value (number, list, or short token).
